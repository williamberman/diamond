...
"""
    ...
    for dataset_train_split_n_example in [512000, 256000, 128000, 64000, 32000, 16000, 8000, 4000, 2000, 1000]:
        for model_type in ["resnet_sweep_1", "resnet_sweep_2", "resnet_sweep_3", "resnet_sweep_4", "resnet_sweep_5"]:
            cmd = rf"""
CUDA_VISIBLE_DEVICES={ctr%args.n_concurrent_processes} torchrun --nproc_per_node=1 --master_port={23456+(ctr%args.n_concurrent_processes)} src/train_classifier.py \
    ...
    --validation_epochs 1 \
    --save_dir /mnt/raid/diamond_v3/{args.game}/classifier_{dataset_train_split_n_example}_{model_type}/ \
    --save_epochs 1 \
    --wandb_name {args.game}_{dataset_train_split_n_example}_{model_type} &
...
"""
    ...
    with open(f"v3_run_scripts/train_classifier_sweep_{args.game}.sh", "w") as f:
        f.write(script)

    os.system(f"chmod +x v3_run_scripts/train_classifier_sweep_{args.game}.sh")
